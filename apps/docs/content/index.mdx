...
Nerve is a unified API for healthtech developers to easily integrate with EHR systems, and build using sandbox data. We
support major platforms like Epic, Cerner, and AthenaHealth and [many more](/docs/integrations)!

## Our Philosophy

Nerve abstracts over the [FHIR](https://hl7.org/fhir/) data model to provide developers a simpler developer interface when reading and writing data to EHR systems. We use FHIR as a ['compile-target'](/nerve-data-model) to ensure interoperability with other health systems while reducing the complexity for developers.
...
## Quick Setup for Sandbox (NextJS App Router)
