...
|------|-------------|----------|--------|
| category | Always "clinical-reference" | Yes | No |
...
| Name | Description | Required | Array |
|------|-------------|----------|--------|
| date | The date and time when the Clinical Reference was attached | No | No |
| format | Format/Content rules for the document (RTF vs HTML) | No | No |
| status | Always "current" | No | No |
...
|------|-------------|-------------|----------|
| author | FHIR ID for the providers who signed or addended the study | true | false |
| category | Must be set to "imaging-result" if specified. Only category or type may be specified in a single query | true | false |
| date | The instant or range of instants when a study was addended or finalized | true | false |
| encounter | Encounter that order is linked to | true | false |
| patient | The subject of the report if a patient | conditional | false |
| period | The instant when the exam was performed | true | false |
| status | The status of the result. Always "current" | true | false |
| subject | The patient that is the subject of the note | conditional | false |
| type | The LOINC or system code for the radiology result. Only category or type may be specified in a single query | true | false |

### Post-filter Request Elements
| Name | Description | Is Optional | Is Array |
|------|-------------|-------------|----------|
| authenticator | The user who signed the study. Reference to the Practitioner resource. Returns only the first user who signed the study | true | false |
| contenttype | Mime content type code. Always "application/pdf" | true | false |
| description | The name of the procedure | true | false |
| identifier | The order identifier from the Epic system | true | false |
