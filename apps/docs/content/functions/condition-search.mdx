...
| clinicalStatus | Status of the condition. FHIR Status: `active`, `resolved` , `inactive` . If no status is specified all active (confirmed) conditions will be returned. *Note: Any FHIR Status not listed in the above table is not returned. | Yes | No |
...
| onsetDate | The date the problem was noted. | true | false |
| recordedDate | The date the condition was recorded. | true | false |
| severity | The priority of the problem. | true | false |
| verificationStatus | If this is on the patient's local chart, the value is "confirmed". If it is outside data, it is "unconfirmed". | true | false |
...
|------|-------------|-------------|----------|
| code | Code for the condition. See the code response element for more information. | true | false |
| onsetDate | The date when the medical problem began for the patient. Only applies to positive medical history.	 | true | false |
| onsetInfo | The date when the medical problem began for the patient. Only applies to positive medical history. Applicable for dates saved as a string instead of a date format. Otherwise, use onset-date.	 | true | false |
| recordedDate | For pertinent negative medical history, the date when the pertinent negative was noted on the patient’s chart.	 | true | false |
| severity | The priority of the problem. | true | false |
| verificationStatus | Use “confirmed” for positive medical history and “refuted” for pertinent negative medical history.	| true | false |
